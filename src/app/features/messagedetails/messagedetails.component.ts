...
export class MessagedetailsComponent implements OnInit, OnDestroy {
  ...
  channelUrl = localStorage.getItem('channelUrl') ?? '';
...
ngOnInit(): void {
  ...
  async initChat(): Promise<void> {
    try {
      ...
      await this.chatService.connectUser(this.specialistId, this.specialistName);
  ...
  isMyMessage(msg: any): boolean {
    return msg.sender?.userId === this.specialistId;
  }
